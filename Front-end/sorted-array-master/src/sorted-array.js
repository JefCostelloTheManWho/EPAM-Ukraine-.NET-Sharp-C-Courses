    ...
    const sortedByRating = () => {
        ...
        for(let i = 1; i < arr.length; i++){
            for(let j= 0; j<arr.length-i;j++){
                let prevReview = parseInt(arr[j].ratingReviews.replace(/\D+/g,""));
                let nextReview = parseInt(arr[j+1].ratingReviews.replace(/\D+/g,""))
    ...
    const sortedByPrice = () => {
        ...
        for(let i = 1; i < arr.length; i++) {
            for(let j = 0; j < arr.length-i;j++) {
                let prevReview;
                let nextReview;
                if(arr[j].price.newUan != undefined) {

                     prevReview = parseInt(arr[j].price.newUan.replace(/\D+/g,""));
                } else {
                     prevReview = parseInt(arr[j].price.replace(/\D+/g,""));

                }
                if(arr[j+1].price.newUan != undefined)
                {
                     nextReview = parseInt(arr[j+1].price.newUan.replace(/\D+/g,""));
                } else {
                     nextReview = parseInt(arr[j+1].price.replace(/\D+/g,""));
                }
                    if(prevReview < nextReview) {
                        let temp = arr[j];
                        arr[j] =arr[j+1];
